...
- (id)initWithSystemId:(uint8_t)systemId componentId:(uint8_t)componentId latitude:(int32_t)latitude longitude:(int32_t)longitude altitude:(int32_t)altitude timeUsec:(uint64_t)timeUsec {
  if ((self = [super init])) {
    mavlink_msg_gps_global_origin_pack(systemId, componentId, &(self->_message), latitude, longitude, altitude, timeUsec);
  }
  return self;
}
...
- (uint64_t)timeUsec {
  return mavlink_msg_gps_global_origin_get_time_usec(&(self->_message));
}

- (NSString *)description {
  return [NSString stringWithFormat:@"%@, latitude=%d, longitude=%d, altitude=%d, timeUsec=%lld", [super description], [self latitude], [self longitude], [self altitude], [self timeUsec]];
}
